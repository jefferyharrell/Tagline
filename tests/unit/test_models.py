...
@pytest.fixture
def media_object(fake_metadata: Dict[str, Any], db_session: Session) -> MediaObject:
    obj = MediaObject(
        id=uuid.uuid4(),
        object_key="test_key",
...
def test_media_object_instantiation(media_object: MediaObject) -> None:
...
def test_media_object_repr(media_object: MediaObject) -> None:
...
def test_model_table_and_columns() -> None:
    ...
    assert MediaObject.__tablename__ == "media_objects"
    # Get column names from table definition
    columns = {col.name for col in MediaObject.__table__.columns}
