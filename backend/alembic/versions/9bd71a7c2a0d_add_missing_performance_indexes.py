...
def upgrade() -> None:
    ...
    op.create_index('idx_media_objects_file_mimetype', 'media_objects', ['file_mimetype'])
    op.create_index('idx_media_objects_file_last_modified', 'media_objects', ['file_last_modified'])
    ...
    op.create_index('idx_media_objects_thumbnail_key', 'media_objects', ['thumbnail_object_key'])
    op.create_index('idx_media_objects_proxy_key', 'media_objects', ['proxy_object_key'])
    
    # Composite indexes for common query patterns
    # ===========================================
    
    # Status + timestamps for dashboard queries
    op.create_index('idx_media_objects_status_created', 'media_objects', 
                   ['ingestion_status', 'created_at'])
    
    # Object key prefix + status for folder navigation
    op.create_index('idx_media_objects_key_prefix_status', 'media_objects', 
                   [sa.text('substring(object_key, 1, 100)'), 'ingestion_status'])
    
    # File type + size for analytics
    op.create_index('idx_media_objects_mimetype_size', 'media_objects', 
