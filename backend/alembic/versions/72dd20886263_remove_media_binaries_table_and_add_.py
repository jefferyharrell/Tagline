...
def upgrade() -> None:
    ...
    op.add_column(
        "media_objects",
        sa.Column("thumbnail_object_key", sa.String(255), nullable=True),
    )
    op.add_column(
        "media_objects", sa.Column("proxy_object_key", sa.String(255), nullable=True)
    )
...
def downgrade() -> None:
    ...
    # Create indexes
    op.create_index(
        "ix_media_binaries_media_object_key", "media_binaries", ["media_object_key"]
    )
